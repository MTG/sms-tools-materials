def _mask_frequencies(tracks, max_freq):
    tracks_masked = np.copy(tracks)
    tracks_masked[tracks_masked > max_freq] = np.nan
    tracks_masked[tracks_masked <= 0] = np.nan
    return tracks_masked

def _plot_waveform(y, fs, title):
    plt.plot(np.arange(y.size) / float(fs), y)
    plt.title(title)
    plt.xlabel("time (sec)")
    plt.ylabel("amplitude")
    plt.xlim([0, y.size / float(fs)])
import numpy as np
