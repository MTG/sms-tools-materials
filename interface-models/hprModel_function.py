...
def main(
    ...
    outputFileSines = os.path.join(output_dir, f"{stem}_hprModel_sines.wav")
    outputFileResidual = os.path.join(output_dir, f"{stem}_hprModel_residual.wav")
    outputFile = os.path.join(output_dir, f"{stem}_hprModel.wav")
    ...
    plt.subplot(3, 1, 2)
    harms = _mask_frequencies(hfreq, maxplotfreq) if hfreq.shape[1] > 0 else None
    PH.plot_spectrogram_with_tracks(plt.gca(), mXr, harms if harms is not None else np.zeros_like(mXr), fs, N, H, max_plot_freq=maxplotfreq, title="harmonics + residual spectrogram")
